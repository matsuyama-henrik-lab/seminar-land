...
プログラミングが初めての人でも大丈夫です。じゅんばんに進めていきましょう。
...
このゲームは、**エンジンのコード（`world.gd` や `player.gd`）をさわらなくても**
...
6. [（応用）かんたんなアイテムを自作する](#6-応用-かんたんなアイテムを自作する)
...
4. 一覧に「 基礎セミナーランド」が出てくるので、ダブルクリックで開きます。
...
旗（🚩）にさわるとステージクリアです。まずはお手本のステージで遊んでみましょう。

---

## 3. 自分のステージを作る（コピーする）

新しいステージは、**からっぽのテンプレート `level_empty.tscn` をコピー**して
...
2. 左の「Scene（シーン）」パネルで、いちばん上の **`World`** ノードをクリック。
...
> 💡 **`Tutorial Levels`**（チュートリアル）は、ふつうの `Levels` とは別の一覧です。
...
| [`garigari.tscn`](items/garigari.md) | まわるノコギリ（危険）。 |
| [`key.tscn`](items/key.md) | カギ。集めるしかけに使えます。 |
| [`platform.tscn`](items/platform.md) | 足場（スクリプトなし）。 |
| [`gravity_flip.tscn`](items/gravity_flip.md) | 重力ぎゃくてん。 |
...
- `items/item_speed.gd` … さわると**速くなる**アイテム
- `items/gravity_flip.gd` … さわると**重力がぎゃくてん**するアイテム
...
| フォルダ／ファイル | 中身 |
| --- | --- |
| `world.gd` / `world.tscn` | ゲーム本体（メニュー・ステージ一覧・ボタン）。**ふつうはさわりません。** |
...
たのしいステージができたら、ぜひ友だちに遊んでもらいましょう！🎮
